...
% Moisture Interaction Code (COSMIC) - Version 1.5 by W. James Shuttlewort 
% and Rafael Rosolem
% COSMIC in matlab version 1.0 by Peng Jinbang 2016/09/20
% The function of this program is assimilation of data from cosmic ray
% neutron probe

%=================================================================================
% Read in parameter file
%=================================================================================
fid = fopen('.\cosmic_parlist','r');
C = textscan(fid,'%f %s','Delimiter','%');
nlayers = C{1,1}(1);                    %Total number of soil layers
nprof= C{1,1}(2);                       %Total number of soil moisture profiles
bd= C{1,1}(3);                          %Dry soil bulk density (g/m3)
vwclat= C{1,1}(4);                      %Volumetric "lattice" water content (m3/m3)
N= C{1,1}(5);                           %High energy neutron flux (-)
alpha= C{1,1}(6);                       %Ratio of Fast Neutron Creation Factor (Soil to Water), alpha (-)
L1= C{1,1}(7);                          %High Energy Soil Attenuation Length (g/cm2)
L2= C{1,1}(8);                          %High Energy Water Attenuation Length (g/cm2)
L3= C{1,1}(9);                          %Fast Neutron Soil Attenuation Length (g/cm2)
L4= C{1,1}(10);                         %Fast Neutron Water Attenuation Length (g/cm2)
...
% Read in input file
%=================================================================================
input_load=load('.\input.dat','r');
[n_layer,n_prof]=size(input_load);
vwc=input_load(1:n_layer,2:n_prof);   %Soil moisture of each layer (m3/m3)
dz=input_load(1:nlayers,1);                 %Soil layers (cm)
...
fprintf('5.result output has finished!\n');
